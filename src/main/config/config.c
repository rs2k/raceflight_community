...
static void resetPidProfile(pidProfile_t *pidProfile)
{
    ...
    pidProfile->wrgyrolpf = 90;
    pidProfile->wpgyrolpf = 90;
    pidProfile->wygyrolpf = 90;
    pidProfile->wrkdlpf = 75;
    pidProfile->wpkdlpf = 75;
    pidProfile->wykdlpf = 70;
    pidProfile->fcquick = 0.007;
    pidProfile->fcrap = 88.0;
    pidProfile->fcpress = 0.0;
 pidProfile->yaw_pterm_cut_hz = 30;
 pidProfile->pitch_pterm_cut_hz = 0;
 pidProfile->witchcraft = 4;
 pidProfile->P_f[ROLL] = 6.600f;
    pidProfile->I_f[ROLL] = 1.475f;
    pidProfile->D_f[ROLL] = 0.190f;
    pidProfile->P_f[PITCH] = 7.600f;
    pidProfile->I_f[PITCH] = 1.475f;
    pidProfile->D_f[PITCH] = 0.210f;
    pidProfile->P_f[YAW] = 8.500f;
    pidProfile->I_f[YAW] = 1.475f;
    pidProfile->D_f[YAW] = 0.170f;
...
#if defined(SPMFC400) || defined(SPMFC384)
 controlRateConfig->rcPitchExpo8 = 20.0f;
 controlRateConfig->rcRollExpo8 = 20.0f;
 controlRateConfig->rcYawExpo8 = 20.0f;
 controlRateConfig->thrMid8 = 50;
 controlRateConfig->thrExpo8 = 0.0f;
 controlRateConfig->dynThrPID = 0;
 controlRateConfig->tpa_breakpoint = 1500;
 controlRateConfig->PitchAcroPlusFactor = 100.0f;
 controlRateConfig->RollAcroPlusFactor = 100.0f;
 controlRateConfig->YawAcroPlusFactor = 20.0f;
 for (uint8_t axis = 0; axis < FLIGHT_DYNAMICS_INDEX_COUNT; axis++) {
  if (axis == 2) {
   controlRateConfig->rates[axis] = 450.0f;
  }
  else {
   controlRateConfig->rates[axis] = 450.0f;
  }
...
static void resetConf(void)
{
 ...
 masterConfig.acc_hardware = 0;
    ...
    masterConfig.rxConfig.max_aux_channels = 4;
 masterConfig.rf_loop_ctrl = DLPF_M4;
 masterConfig.arm_method = ALWAYS_DOUBLE;
 ...
 parseRcChannels("TAER1234", &masterConfig.rxConfig);
    ...
    masterConfig.fsMode = FS_FALL_OUT_OF_SKY;
    masterConfig.fsCondition = FS_NO_RX_PULSE_AND_LOW_THROTTLE;
    masterConfig.fsStg1 = 20;
    masterConfig.fsStg2 = 10;
    masterConfig.fsLowThrottle = 950;
...
#endif
#if defined(SPMFC384) || defined(SPMFC400)
    masterConfig.rxConfig.spektrum_sat_bind=9;
 masterConfig.batteryConfig.vbatscale = 66;
 masterConfig.batteryConfig.vbatdivider = 216;
 masterConfig.batteryConfig.currentMeterScale = 250;
 featureSet(FEATURE_VBAT);
 featureSet(FEATURE_CURRENT_METER);
 parseRcChannels("TAER1234", &masterConfig.rxConfig);
 masterConfig.rf_loop_ctrl = DLPF_H8;
    currentProfile->modeActivationConditions[0].modeId = BOXARM;
    currentProfile->modeActivationConditions[0].auxChannelIndex = 1;
    currentProfile->modeActivationConditions[0].range.startStep = 28;
    currentProfile->modeActivationConditions[0].range.endStep = 48;
#endif
...
#if defined(STM32F40_41xxx)
  #ifndef CONFIG_FLASH_SECTOR
    #define CONFIG_FLASH_SECTOR FLASH_Sector_3
  #endif
                status = FLASH_EraseSector(CONFIG_FLASH_SECTOR, VoltageRange_3);
#elif defined (STM32F411xE)
  #ifndef CONFIG_FLASH_SECTOR
    #define CONFIG_FLASH_SECTOR FLASH_Sector_3
  #endif
