...
#define ID_VERT_SPEED         0x30 

#define GPS_BAD_QUALITY       300
#define GPS_MAX_HDOP_VAL      9999
#define DELAY_FOR_BARO_INITIALISATION (5 * 1000) 
#define BLADE_NUMBER_DIVIDER  5 
...
{
    uint16_t throttleForRPM = rcCommandUsed[THROTTLE] / BLADE_NUMBER_DIVIDER;
    sendDataHead(ID_RPM);
...
static void sendSatalliteSignalQualityAsTemperature2(void)
{
    ...
    if (GPS_hdop > GPS_BAD_QUALITY && ( (cycleNum % 16 ) < 8)) {
...
static void GPStoDDDMM_MMMM(int32_t mwiigps, gpsCoordinateDDDMMmmmm_t *result)
{
    ...
    absgps = (absgps - deg * GPS_DEGREES_DIVIDER) * 60;        
    min    = absgps / GPS_DEGREES_DIVIDER;                     
...
void handleFrSkyTelemetry(rxConfig_t *rxConfig, uint16_t deadband3d_throttle)
{
    ...
    if ((cycleNum % 4) == 0) {      
        if (lastCycleTime > DELAY_FOR_BARO_INITIALISATION) { 
            sendBaro();
        }
        sendHeading();
        sendTelemetryTail();
