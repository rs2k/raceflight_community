 ...
 * the signals don't get mixed up.  When RaceFlight transmits it should not receive it's own transmission.
 *
 * Connect as follows:
...
void hottPrepareGPSResponse(HOTT_GPS_MSG_t *hottGPSMessage)
{
    ...
    uint16_t speed = (GPS_speed * 36) / 100; 
    ...
    uint16_t hottGpsAltitude = (GPS_altitude / 10) + HOTT_GPS_ALTITUDE_OFFSET; 
