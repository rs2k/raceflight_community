...
float rcCommand[4];           
float rcCommandUsed[4];       
float trueRcCommandF[4];      
...
void applyStepAdjustment(controlRateConfig_t *controlRateConfig, uint8_t adjustmentFunction, int delta) {
    ...
    switch(adjustmentFunction) {
        ...
        case ADJUSTMENT_THROTTLE_EXPO:
            newValue = constrain((int)controlRateConfig->thrExpo8 + delta, 0, 100); 
        ...
        case ADJUSTMENT_PITCH_P:
            if (IS_PID_CONTROLLER_FP_BASED(pidProfile->pidController)) {
                newFloatValue = constrainf(pidProfile->P_f[PIDPITCH] + (float)(delta / 100.0f), 0, 100); 
                pidProfile->P_f[PIDPITCH] = newFloatValue;
                blackboxLogInflightAdjustmentEventFloat(ADJUSTMENT_PITCH_P, newFloatValue);
            } else {
                newValue = constrain((int)pidProfile->P8[PIDPITCH] + delta, 0, 200); 
        ...
        case ADJUSTMENT_ROLL_P:
            if (IS_PID_CONTROLLER_FP_BASED(pidProfile->pidController)) {
                newFloatValue = constrainf(pidProfile->P_f[PIDROLL] + (float)(delta / 100.0f), 0, 100); 
                pidProfile->P_f[PIDROLL] = newFloatValue;
                blackboxLogInflightAdjustmentEventFloat(ADJUSTMENT_ROLL_P, newFloatValue);
            } else {
                newValue = constrain((int)pidProfile->P8[PIDROLL] + delta, 0, 200); 
        ...
        case ADJUSTMENT_PITCH_I:
            if (IS_PID_CONTROLLER_FP_BASED(pidProfile->pidController)) {
                newFloatValue = constrainf(pidProfile->I_f[PIDPITCH] + (float)(delta / 100.0f), 0, 100); 
                pidProfile->I_f[PIDPITCH] = newFloatValue;
                blackboxLogInflightAdjustmentEventFloat(ADJUSTMENT_PITCH_I, newFloatValue);
            } else {
                newValue = constrain((int)pidProfile->I8[PIDPITCH] + delta, 0, 200); 
        ...
        case ADJUSTMENT_ROLL_I:
            if (IS_PID_CONTROLLER_FP_BASED(pidProfile->pidController)) {
                newFloatValue = constrainf(pidProfile->I_f[PIDROLL] + (float)(delta / 100.0f), 0, 100); 
                pidProfile->I_f[PIDROLL] = newFloatValue;
                blackboxLogInflightAdjustmentEventFloat(ADJUSTMENT_ROLL_I, newFloatValue);
            } else {
                newValue = constrain((int)pidProfile->I8[PIDROLL] + delta, 0, 200); 
        ...
        case ADJUSTMENT_PITCH_D:
            if (IS_PID_CONTROLLER_FP_BASED(pidProfile->pidController)) {
                newFloatValue = constrainf(pidProfile->D_f[PIDPITCH] + (float)(delta / 1000.0f), 0, 100); 
                pidProfile->D_f[PIDPITCH] = newFloatValue;
                blackboxLogInflightAdjustmentEventFloat(ADJUSTMENT_PITCH_D, newFloatValue);
            } else {
                newValue = constrain((int)pidProfile->D8[PIDPITCH] + delta, 0, 200); 
        ...
        case ADJUSTMENT_ROLL_D:
            if (IS_PID_CONTROLLER_FP_BASED(pidProfile->pidController)) {
                newFloatValue = constrainf(pidProfile->D_f[PIDROLL] + (float)(delta / 1000.0f), 0, 100); 
                pidProfile->D_f[PIDROLL] = newFloatValue;
                blackboxLogInflightAdjustmentEventFloat(ADJUSTMENT_ROLL_D, newFloatValue);
            } else {
                newValue = constrain((int)pidProfile->D8[PIDROLL] + delta, 0, 200); 
        ...
        case ADJUSTMENT_YAW_P:
            if (IS_PID_CONTROLLER_FP_BASED(pidProfile->pidController)) {
                newFloatValue = constrainf(pidProfile->P_f[PIDYAW] + (float)(delta / 100.0f), 0, 100); 
                pidProfile->P_f[PIDYAW] = newFloatValue;
                blackboxLogInflightAdjustmentEventFloat(ADJUSTMENT_YAW_P, newFloatValue);
            } else {
                newValue = constrain((int)pidProfile->P8[PIDYAW] + delta, 0, 200); 
        ...
        case ADJUSTMENT_YAW_I:
            if (IS_PID_CONTROLLER_FP_BASED(pidProfile->pidController)) {
                newFloatValue = constrainf(pidProfile->I_f[PIDYAW] + (float)(delta / 100.0f), 0, 100); 
                pidProfile->I_f[PIDYAW] = newFloatValue;
                blackboxLogInflightAdjustmentEventFloat(ADJUSTMENT_YAW_I, newFloatValue);
            } else {
                newValue = constrain((int)pidProfile->I8[PIDYAW] + delta, 0, 200); 
        ...
        case ADJUSTMENT_YAW_D:
            if (IS_PID_CONTROLLER_FP_BASED(pidProfile->pidController)) {
                newFloatValue = constrainf(pidProfile->D_f[PIDYAW] + (float)(delta / 1000.0f), 0, 100); 
                pidProfile->D_f[PIDYAW] = newFloatValue;
                blackboxLogInflightAdjustmentEventFloat(ADJUSTMENT_YAW_D, newFloatValue);
            } else {
                newValue = constrain((int)pidProfile->D8[PIDYAW] + delta, 0, 200); 
