 ...
 * along with RaceFlight.  If not, see <http://www.gnu.org/licenses/>.
 */ 
#define FUNNYTEXT "lololololololololololol"
#include <stdbool.h>
#include <stdint.h>
#include <stdlib.h>
#include <stdarg.h>
...
#endif
static void cliRfblBind(char *cmdline);
static void rfCustomRfblBind(char *cmdline);
static void cliResetDfu(char *cmdline);
static void cliResetRfbl(char *cmdline);
static void cliResetRecovery(char *cmdline);
...
static const char * const mixerNames[] = {
    ...
    "ATAIL4", "CUSTOM", "CUSTOMAIRPLANE", "CUSTOMTRI", "QUADX1234", "QUADX2143", "QUADX1", "QUADX2", "QUADXL1234", "QUADWIDEX1234", NULL
};
#endif
...
const clicmd_t cmdTable[] = {
    CLI_COMMAND_DEF("rfblBind", "RFBL will bind spektrum satellite on next powerup", NULL, cliRfblBind),
    CLI_COMMAND_DEF("resetDfu", "Restart into DFU mode", NULL, cliResetDfu),
    CLI_COMMAND_DEF("resetRfbl", "Restart into RFBL mode", NULL, cliResetRfbl),
    CLI_COMMAND_DEF("resetRecovery", "Restart into Recovery mode", NULL, cliResetRecovery),
...
static const char * const lookupTableFailsafeMode[] = {
 "FS_FALL_OUT_OF_SKY",
};
static const char * const lookupTableFailsafeCondition[] = {
 "FS_NO_RX_PULSE",
 "FS_LOW_THROTTLE",
 "FS_NO_RX_PULSE_AND_LOW_THROTTLE",
};
...
static const lookupTableEntry_t lookupTables[] = {
    ...
    { lookupTableFailsafeMode, sizeof(lookupTableFailsafeMode) / sizeof(char *) },
    { lookupTableFailsafeCondition, sizeof(lookupTableFailsafeCondition) / sizeof(char *) },
};
...
const clivalue_t valueTable[] = {
 { "mid_rc", VAR_UINT16 | MASTER_VALUE, &masterConfig.rxConfig.midrc, .config.minmax = { 1200, 1700 } },
    ...
    { "cfsstg1", VAR_UINT8 | MASTER_VALUE, &masterConfig.fsStg1, .config.minmax = { 0, 200 } },
    { "cfsstg2", VAR_UINT8 | MASTER_VALUE, &masterConfig.fsStg2, .config.minmax = { 0, 200 } },
    { "wfslowthr", VAR_UINT16 | MASTER_VALUE, &masterConfig.fsLowThrottle, .config.minmax = { 0, 1500 } },
    { "cfsmode", VAR_UINT8 | MASTER_VALUE | MODE_LOOKUP, &masterConfig.fsMode, .config.lookup = { TABLE_FAILSAFE_MODE } },
    { "cfscond", VAR_UINT8 | MASTER_VALUE | MODE_LOOKUP, &masterConfig.fsCondition, .config.lookup = { TABLE_FAILSAFE_CONDITION } },
...
void ProcessCommand(char *inString)
{
 ...
 else if (!strcmp("resetrfbl", inString))
 {
...
static void oneWireCommandDisconnect(char *extraArgs) {
    UNUSED(extraArgs);
    if (esc1WireStatus() >= 0) {
        esc1WireDisconnect();
    }
...
static void rfCustom1Wire(char *command) {
 ...
 bzero(rf_custom_out_buffer, 255);
    ...
    } else if (checkCommand(command, "disconnect")) {
        oneWireCommandDisconnect(args);
...
UNUSED(cmdline);
  ...
  sendSpektrumBind();
  cliPrintf("Bind command sent to telemetry RX");
#else
  cliPrintf("You don't have RFBL or a Spektrum telemetry build. This makes me a sad panda. :(");
#endif
}
static void rfCustomRfblBind(char *cmdline)
{
