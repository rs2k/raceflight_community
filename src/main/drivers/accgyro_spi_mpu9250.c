 ...
 * Kalyn Doerr (RS2K) - Robust rewrite - RaceFlight!
*/
...
#else 
	spiSetDivisor(MPU9250_SPI_INSTANCE, SPI_ULTRAFAST_CLOCK); 
#endif
...
static void mpu9250AccAndGyroInit(uint8_t lpf) {
    ...
    if (lpf >= DLPF_UH1) {
        ...
        verifympu9250WriteRegister(MPU_RA_GYRO_CONFIG, INV_FSR_2000DPS << 3 | FCB_8800_32);
    } else if (lpf >= DLPF_H16) {
        
        verifympu9250WriteRegister(MPU_RA_GYRO_CONFIG, INV_FSR_2000DPS << 3 | FCB_3600_32);
    } else {
        
        verifympu9250WriteRegister(MPU_RA_GYRO_CONFIG, INV_FSR_2000DPS << 3 | FCB_DISABLED);
    }

    if (lpf == DLPF_L1) {
        verifympu9250WriteRegister(MPU_RA_CONFIG, 1); 
    } else if (lpf <= DLPF_M8) {
        verifympu9250WriteRegister(MPU_RA_CONFIG, 0); 
    } else if (lpf <= DLPF_H8) {
        verifympu9250WriteRegister(MPU_RA_CONFIG, 7); 
    }
    

	verifympu9250WriteRegister(MPU_RA_SMPLRT_DIV, mpuDividerDrops); 

	verifympu9250WriteRegister(MPU_RA_ACCEL_CONFIG, INV_FSR_16G << 3);
	verifympu9250WriteRegister(MPU_RA_INT_PIN_CFG, 0 << 7 | 0 << 6 | 0 << 5 | 1 << 4 | 0 << 3 | 0 << 2 | 1 << 1 | 0 << 0);  

#if defined(USE_MPU_DATA_READY_SIGNAL)
	verifympu9250WriteRegister(MPU_RA_INT_ENABLE, 0x01); 
#endif
