...
{
    ENABLE_MPU6000;
    spiTransferByte(MPU6000_SPI_INSTANCE, reg | 0x80); 
...
static void mpu6000AccAndGyroInit(uint8_t lpf) {
	...
	verifympu6000WriteRegister(MPU_RA_PWR_MGMT_1, 0x0B); 
    ...
    verifympu6000WriteRegister(MPU_RA_SMPLRT_DIV, mpuDividerDrops);
    ...
    verifympu6000WriteRegister(MPU_RA_INT_PIN_CFG,  0 << 7 | 0 << 6 | 0 << 5 | 1 << 4 | 0 << 3 | 0 << 2 | 0 << 1 | 0 << 0);  

#if defined(USE_MPU_DATA_READY_SIGNAL)
    mpu6000WriteRegister(MPU_RA_INT_ENABLE, MPU_RF_DATA_RDY_EN); 
    delayMicroseconds(100);
    verifympu6000WriteRegister(MPU_RA_PWR_MGMT_1, 0x0B); 
    verifympu6000WriteRegister(MPU_RA_INT_ENABLE, MPU_RF_DATA_RDY_EN); 
#endif

    
    if (lpf == DLPF_L1) {
        verifympu6000WriteRegister(MPU6000_CONFIG, 1); 
    } else if (lpf <= DLPF_M8) {
        verifympu6000WriteRegister(MPU6000_CONFIG, 0); 
    } else {
        verifympu6000WriteRegister(MPU6000_CONFIG, 7); 
    }
