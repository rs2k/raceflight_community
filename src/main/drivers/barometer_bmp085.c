...
STATIC_UNIT_TESTED uint16_t bmp085_ut;
STATIC_UNIT_TESTED uint32_t bmp085_up;
...
#endif
    delay(20);
    ack = i2cRead(BMP085_I2C_INSTANCE, BMP085_I2C_ADDR, BMP085_CHIP_ID__REG, 1, &data);
    if (ack) {
        bmp085.chip_id = BMP085_GET_BITSLICE(data, BMP085_CHIP_ID);
        bmp085.oversampling_setting = 3;
        if (bmp085.chip_id == BMP085_CHIP_ID) {
            i2cRead(BMP085_I2C_INSTANCE, BMP085_I2C_ADDR, BMP085_VERSION_REG, 1, &data);
            bmp085.ml_version = BMP085_GET_BITSLICE(data, BMP085_ML_VERSION);
            bmp085.al_version = BMP085_GET_BITSLICE(data, BMP085_AL_VERSION);
...
static int32_t bmp085_get_temperature(uint32_t ut)
{
    ...
    temperature = ((bmp085.param_b5 * 10 + 8) >> 4);
    return temperature;
}
static int32_t bmp085_get_pressure(uint32_t up)
{
...
#endif
    i2cWrite(BMP085_I2C_INSTANCE, BMP085_I2C_ADDR, BMP085_CTRL_MEAS_REG, BMP085_T_MEASURE);
}
static void bmp085_get_ut(void)
{
    uint8_t data[2];
...
#endif
    i2cRead(BMP085_I2C_INSTANCE, BMP085_I2C_ADDR, BMP085_ADC_OUT_MSB_REG, 2, data);
...
static void bmp085_start_up(void)
{
    ...
    isConversionComplete = false;
#endif
    i2cWrite(BMP085_I2C_INSTANCE, BMP085_I2C_ADDR, BMP085_CTRL_MEAS_REG, ctrl_reg_data);
}
static void bmp085_get_up(void)
{
    uint8_t data[3];
...
#endif
    i2cRead(BMP085_I2C_INSTANCE, BMP085_I2C_ADDR, BMP085_ADC_OUT_MSB_REG, 3, data);
...
{
    uint8_t data[22];
    i2cRead(BMP085_I2C_INSTANCE, BMP085_I2C_ADDR, BMP085_PROM_START__ADDR, BMP085_PROM_DATA__LEN, data);
