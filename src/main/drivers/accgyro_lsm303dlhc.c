...
#ifndef LSM303DLHC_I2C_INSTANCE
#define LSM303DLHC_I2C_INSTANCE I2C_DEVICE
#endif
...
{
    i2cWrite(LSM303DLHC_I2C_INSTANCE, LSM303DLHC_ACCEL_ADDRESS, CTRL_REG5_A, BOOT);

    delay(100);

    i2cWrite(LSM303DLHC_I2C_INSTANCE, LSM303DLHC_ACCEL_ADDRESS, CTRL_REG1_A, ODR_1344_HZ | AXES_ENABLE);

    delay(10);

    i2cWrite(LSM303DLHC_I2C_INSTANCE, LSM303DLHC_ACCEL_ADDRESS, CTRL_REG4_A, FULLSCALE_4G);
...
static bool lsm303dlhcAccRead(int16_t *gyroADC)
{
    ...
    bool ack = i2cRead(LSM303DLHC_I2C_INSTANCE, LSM303DLHC_ACCEL_ADDRESS, AUTO_INCREMENT_ENABLE | OUT_X_L_A, 6, buf);
...
bool lsm303dlhcAccDetect(acc_t *acc)
{
    ...
    ack = i2cRead(LSM303DLHC_I2C_INSTANCE, LSM303DLHC_ACCEL_ADDRESS, LSM303DLHC_STATUS_REG_A, 1, &status);
    if (!ack)
        return false;
