...
int16_t applyTPA(float throttle) {
 float attenuation;
    static float curveValuesC[9] = {0.0f, 0.0f, 0.15f, 0.20f, 0.20f, 0.25f, 0.30f, 0.3f, 0.3f};
...
void annexCode(void)
{
  ...
  __enable_irq();
    ...
    if (IS_RC_MODE_ACTIVE(BOXLANDING)) {
     FullKiLatched = false;
    }
...
void mwDisarm(void)
{
    if (ARMING_FLAG(ARMED)) {
        ...
        rx_watchdog_init(Watchdog_Timeout_32s);
        FullKiLatched = false;
#ifdef BLACKBOX
...
void mwArm(void)
{
 ...
 if (IS_RC_MODE_ACTIVE(BOXDELETEFLASH)) {
  return;
 }
    if (ARMING_FLAG(OK_TO_ARM)) {
        if (ARMING_FLAG(ARMED)) {
   ...
   return;
        ...
        if (!ARMING_FLAG(PREVENT_ARMING)) {
         doubleOnce = true;
            ...
            rx_watchdog_init(Watchdog_Timeout_2s);
            timeArmedAt = micros();
...
void MainPidLoop(void)
{
 ...
 if (cycleTime > (targetLooptime * 1.5))
 {
