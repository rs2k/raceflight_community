...
F4_TARGETS = SOULF4 REVO REVOLT SPARKY2 REVONANO REVONANO_VB ALIENFLIGHTF4 BLUEJAYF4 BLUEJAYF4_REV3 VRCORE QUANTON KKNGF4 KKNGF4_6500 HOOLIGAN PRACER

F405_TARGETS = SOULF4 REVO REVOLT SPARKY2 ALIENFLIGHTF4 BLUEJAYF4 BLUEJAYF4_REV3 VRCORE QUANTON KKNGF4 KKNGF4_6500 HOOLIGAN
F4_256K_TARGETS	 = SOULF4 REVO REVO_OPBL REVOLT SPARKY2 REVONANO REVONANO_VB ALIENFLIGHTF4 BLUEJAYF4 BLUEJAYF4_REV3 VRCORE QUANTON QUANTON_OPBL AQ32_V2 PRACER REVONANO_VBKKNGF4 KKNGF4_6500 KKNGF4 HOOLIGAN
F4_512K_TARGETS	 =
...
FC_VER_MAJOR := $(shell grep " FC_VERSION_MAJOR" src/main/version.h | gawk '{print $$3}' )
FC_VER_MINOR := $(shell grep " FC_VERSION_MINOR" src/main/version.h | gawk '{print $$3}' )
FC_VER_PATCH := $(shell grep " FC_VERSION_PATCH_LEVEL" src/main/version.h | gawk '{print $$3}' )
FC_VER_LETTER := $(shell grep " FC_VERSION_LETTER" src/main/version.h | gawk '{print $$3}' )
FC_VER_TYPE := $(shell grep " FC_VERSION_TYPE" src/main/version.h | gawk '{print $$3}' )
FC_BUILD_DATE := $(shell date +"%y%m%d")
FC_BUILD_NUMBER := $(shell grep " FC_BUILD_NUMBER" src/main/version.h | gawk '{print $$3}' )

FC_VER := $(FC_BUILD_DATE)_$(FC_VER_MAJOR).$(FC_VER_MINOR).$(FC_VER_PATCH)_$(FC_VER_TYPE)$(FC_BUILD_NUMBER)
...
ifeq ($(TARGET),$(filter $(TARGET),RMDO BEEROTOR IRCFUSIONF3))
# RMDO, BEEROTOR and IRCFUSIONF3 are a VARIANT of SPRACINGF3
...
ifeq ($(TARGET),$(filter $(TARGET), $(F411_TARGETS) $(F446_TARGETS)))
...
ifeq ($(TARGET),$(filter $(TARGET), KKNGF4_6500))
TARGET_FLAGS := $(TARGET_FLAGS) -DKKNGF4 -DALT_6500
TARGET_DIR = $(ROOT)/src/main/target/KKNGF4
TARGET_SRC = $(notdir $(wildcard $(TARGET_DIR)/*.c))
endif

ifeq ($(TARGET),$(filter $(TARGET), REVONANO_VB))
TARGET_FLAGS := $(TARGET_FLAGS) -DREVONANO -DREVONANO_VB
...
KKNGF4_6500_SRC = $(STM32F4xx_COMMON_SRC) \
		   drivers/accgyro_spi_mpu9250.c \
		   drivers/barometer_ms5611.c \
		   $(HIGHEND_SRC) \
		   $(COMMON_SRC) \
...
REVONANO_SRC = $(STM32F411_COMMON_SRC) \
		   drivers/accgyro_spi_mpu9250.c \
		   drivers/barometer_ms5611.c \
		   drivers/compass_hmc5883l.c \
		   $(HIGHEND_SRC) \
...
HOOLIGAN_SRC = $(STM32F4xx_COMMON_SRC) \
		   drivers/accgyro_spi_mpu9250.c \
		   drivers/barometer_ms5611.c \
		   drivers/compass_hmc5883l.c \
		   $(HIGHEND_SRC) \
...
OPTIMIZE	 = -$(OPT)
else ifeq ($(TARGET),$(filter $(TARGET), SOULF4 REVO REVOLT REVONANO REVONANO_VB ALIENFLIGHTF4 BLUEJAYF4 BLUEJAYF4_REV3 VRCORE KKNGF4 KKNGF4_6500 HOOLIGAN PRACER))
OPTIMIZE	 = -$(OPT)
else
OPTIMIZE	 = -$(OPT)
endif
...
copy_kalyn:
	mkdir -p /cygdrive/c/Users/Kalyn/Dropbox/public/raceflight_builds/$(FC_VER)
	find obj/ -name \*.bin -exec cp {} /cygdrive/c/Users/Kalyn/Dropbox/public/raceflight_builds/$(FC_VER)/ \;
